...
export function assertBrandCatalog(
    value: unknown,
): asserts value is BrandCrawlCatalog {
    if (!isRecord(value)) {
        throw new Error('Brand catalog phải là một JSON object.');
    }
    if (value['schemaVersion'] !== 1 || value['sourcePlatform'] !== 'tiki') {
        throw new Error(
            'Brand catalog không đúng schemaVersion hoặc sourcePlatform.',
        );
    }
...
function assertBrand(value: unknown): asserts value is CrawledBrand {
    ...
    if (
        ...
        throw new Error(
            `Brand ${value['externalBrandId']} có aliases/categories không hợp lệ.`,
        );
    }
    if (!isRecord(value['country'])) {
        throw new Error(
            `Brand ${value['externalBrandId']} thiếu dữ liệu country.`,
        );
    }
}
